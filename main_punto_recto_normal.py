...
def main():
    ...
    localidad = st.text_input("Dirección:")
    ...
    st.subheader("**Medidas:**")  # Subtítulo más grande y en negrita
    ...
    # Listas de valores disponibles ordenadas
    lineas_disponibles = sorted(precios.keys())
    brazos_disponibles = sorted(next(iter(precios.values())).keys())

    # Seleccionar el próximo valor más alto disponible para linea y brazo
    ...
    st.subheader("**Tejadillo**")  # Subtítulo más grande y en negrita
    # Checkbox para Tejadillo
    ...
    if tejadillo:
        ...
        # Diccionario de precios del tejadillo
        precios_tejadillo = {
            4.0: 450,
            5.0: 550,
            6.0: 620,
        ...
        st.write(f"Costo del tejadillo para {medida_tejadillo} m: EUR{precio_tejadillo}")
    ...
    st.subheader("**Motores**")  # Subtítulo más grande y en negrita
    # Lista desplegable para seleccionar motor
    opcion_motor = st.selectbox("Motor", ["Motor Pulsador", "Motor Mando", "No usar Motores"])
    precios_motor = {"Motor Pulsador": 200, "Motor Mando": 400, "No usar Motores": 0}
    precio_motor = precios_motor[opcion_motor]
    st.write(f"Costo del Motor {opcion_motor}: ${precio_motor}")
    



    #**********************************
    st.subheader("**Faldon**")  # Subtítulo más grande y en negrita
    # Checkbox para Tejadillo
    ...
    if faldon:
        ...
        st.write(f"Costo del Motor {opcion_faldon}: EUR{precio_faldon}")
        
        st.write(f"Costo del faldon para {medida_faldon} m")
    ...
    if cliente and localidad and telefono:
        if st.button("Hacer Presupuesto"):
            if any([cliente, localidad, telefono]):
                ...
                # Define widths for the two columns
                ...
                # First column
                pdf.set_x(10)  # Set X to 10 from the left margin
                ...
                pdf.cell(left_column_width, 10, 'restaurantemadeinsud@gmail.com', 0, 1, 'L')
                
                
        

                # Second column
                pdf.set_xy(100, 40)  # Move to the right column
                pdf.cell(right_column_width, 10, 'Presupuestar:', 0, 1, 'L')
                pdf.set_xy(100, 50)  # Move to the right column
                pdf.cell(right_column_width, 10, f'   {cliente}', 0, 1, 'L')
                pdf.set_xy(100, 60)  # Move to the right column
                pdf.cell(right_column_width, 10, f'   {localidad}', 0, 1, 'L')
                pdf.set_xy(100, 70)  # Move to the right column
                pdf.cell(right_column_width, 10, 'Grau i Platja, Valencia', 0, 1, 'L')
                pdf.set_xy(100, 80)  # Move to the right column
                pdf.cell(right_column_width, 10, f'   +34 {telefono}', 0, 1, 'L')
            
                

                pdf.line(10, 118, 180, 118)  # Dibuja una línea desde el punto (10, 36) hasta (200, 36)
                ...
                x_inicio = 10  # Margen izquierdo
                y_inicio = 150  # Altura donde comienza la tabla

                # Anchos de columnas
                ancho_descripcion = 90  # Ancho más grande para "DESCRIPCIÓN"
                ancho_ud = 20  # Ancho para "UD"
                ancho_precio = 30  # Ancho para "PRECIO"
                ancho_total = 30  # Ancho para "TOTAL"
                ...
                y_inicio += 10  # Move down after headers
